...
lockPwd.addEventListener("keydown", (e) => { if (e.key === "Enter" && lockPwd.value.trim() === "Zone01") { document.getElementById("lock-screen").style.transition = "opacity 0.5s"; document.getElementById("lock-screen").style.opacity = "0"; setTimeout(() => { document.getElementById("lock-screen").style.display = "none"; const dt = document.getElementById("desktop"); dt.style.display = "block"; dt.style.opacity = "0"; dt.style.transition = "opacity 0.5s"; setTimeout(() => dt.style.opacity = "1", 50); setTimeout(() => notify("Mission: 4 cibles. Recherchez progressivement!", 8000), 1000); }, 500); } else if (e.key === "Enter") { lockErr.textContent = "Acces refuse. Indice: Ecole de Tech a Rouen."; lockPwd.value = ""; lockPwd.style.borderColor = "#f38ba8"; setTimeout(() => lockPwd.style.borderColor = "rgba(255,255,255,0.2)", 1000); } });
